...
srun --partition=xr_research --mpi=pmi2 --gres=gpu:1 -n 1 --ntasks-per-node=1 --job-name=fixorb-office4 --kill-on-bad-exit=1 python -W ignore run.py configs/Replica/office4orb.yaml >log/office4orbfix.log
srun --partition=xr_research --mpi=pmi2 --gres=gpu:1 -n 1 --ntasks-per-node=1 --job-name=fixorb-office4-prior --kill-on-bad-exit=1 python -W ignore run.py configs/Replica/office4orb-spcd-KL.yaml >log/office4orbfix-spcd-KL.log